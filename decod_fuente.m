function nombre_archivo_salida = decod_fuente(arreglo_entrada, nombre_archivo_salida, diccionario, cant_distintos)
    ...
    while indice <= size(arreglo_entrada, 2)
       ...
       % Se verifica si la cadena existe en el mapa
       if isKey(mapaDecodificacion, simbolo_codificado)
           ...
           %símbolo actual
            ...
            if cantidad_caracteres_hallados >= cant_totales_recuperada
                %se llegó al final del archivo original
                break;
            end 
       ...
       % Si se encuentra una cadena inválida
       elseif length(simbolo_codificado) > cant_distintos
            fwrite(archivo_salida, '?', 'char'); 
            
            % Se resettea la cadena para intentar volver a sincronizar con los nuevos bits
            simbolo_codificado = ''; 
       end
